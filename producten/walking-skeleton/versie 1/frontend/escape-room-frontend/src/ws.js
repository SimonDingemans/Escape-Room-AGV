    ...
    _addMessage: (rawMessage, messagePrefix = '') => {
        set((state) => {
            ...
            try {
                ...
                const appendToMessages = (subject, newMessage) => {
                    const oldMessages = messages[subject] ?? [];
                    messages[subject] = [...oldMessages, newMessage];
                }

                if (Array.isArray(message)) {
                    for (let i = 0; i < message.length; i++) {
                        appendToMessages(message[i].subject, JSON.stringify(message[i]));
                    }
                } else {
                    appendToMessages(message.subject, message);
                }
                // eslint-disable-next-line no-unused-vars
            } catch (e) {
                const oldMessages = messages['noSubject'] ?? [];
                messages['noSubject'] = [...oldMessages, fullMessage];
            }
